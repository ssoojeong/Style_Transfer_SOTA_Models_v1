...
def main(model_id, image_path, step_list, prompt_name_backward, forward_prompt, backward_prompts, out_img_save_path, cross_attention_injection_ratio, self_attention_injection_ratio, pretrained_model_name_or_path):
...
def inversion(model_id, content_img_dir, out_path_base, train_img_path, train_step, cross_attention_injection_ratio, self_attention_injection_ratio, pretrained_model_name_or_path):
    """
    ...
    for file in sorted(os.listdir(content_img_dir)):
        content_path = os.path.join(content_img_dir, file)
        if has_valid_extension(content_path, extensions):
            content_name = content_path.split('/')[-1]
            out_img_save_path = os.path.join(out_path_base, content_name)
            prompt_name_backward = "s2"
            forward_prompt = ""
            backward_prompts = ["", "<s2>"]
            step_list = [train_step]
            main(model_id, content_path, step_list, prompt_name_backward, forward_prompt, backward_prompts, out_img_save_path, cross_attention_injection_ratio, self_attention_injection_ratio, pretrained_model_name_or_path)
