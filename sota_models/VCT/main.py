...
def parse_args():
    ...
    parser.add_argument(
        "--style_nums",  #52,20,38,83,79 - wikiart // 1003,1004,1005 - inst
        type=str, 
        default='10', 
    )
    ...
    parser.add_argument(
        "--style_path", 
        type=str, 
        default='../../dataset/style/wikiart', 
    )

    parser.add_argument(
        "--content_path", 
        type=str, 
        default='../../dataset/content/custom5', 
    )

    parser.add_argument(
        "--output_image_path",
        type=str,
        default="../../stylized_images/VCT",
    ...
    parser.add_argument(
        "--use_l1",
        action="store_true",
        help="Use l1 latent loss for training.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="../../resources/models/sd-v1-4",
    ...
    parser.add_argument(
        ...
        type=str,
        default="<s2>",
        help="A token to use as a placeholder for each concept position. By default it is set to <s2>",
    )
    parser.add_argument(
        "--initializer_token", 
        type=str, 
...
def multi_concept_inversion(args):

    # obtain the concept folder name form its path
    concept_folder_name = os.path.split(os.path.normpath(args.style_path))[-1] or os.path.splitdrive(os.path.normpath(args.style_path))[-1]
    ...
    args.output_dir = os.path.join(args.output_dir, 'style_'+concept_folder_name)
    os.makedirs(args.output_dir, exist_ok=True)
    os.makedirs(args.output_image_path, exist_ok=True)

    if os.path.exists(os.path.join(args.style_path, ".ipynb_checkpoints")):
        shutil.rmtree(os.path.join(args.style_path, ".ipynb_checkpoints"))
    ...
    if accelerator.is_main_process:
        ...
        save_progress(tokenizer, text_encoder, accelerator, args, global_step)
...
if __name__ == "__main__":
    ...
    for style_num in sorted(opt.style_nums.split(',')):
        opt2.style_path = os.path.join(opt.style_path, style_num)
        opt2.output_image_path = os.path.join(opt.output_image_path, f'content_{content_name}','style_'+style_num)
        
        args = multi_concept_inversion(opt2)
        pivot_turning_inversion.inversion(args.output_dir, args.content_path, args.output_image_path, args.style_path, args.max_train_steps, args.cross_attention_injection_ratio, args.self_attention_injection_ratio, args.pretrained_model_name_or_path)
