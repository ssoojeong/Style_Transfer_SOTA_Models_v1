...
def init_accelerator_and_logger(logger, opt):
    style_num = opt.style_path.split('/')[-2] 
    logging_dir = ospj(opt.output_dir, 'style_'+style_num, opt.logging_dir)
    ...
    style_num = opt.style_path.split('/')[-2] 
    os.makedirs(ospj(opt.output_dir, 'style_'+style_num, "embedding"), exist_ok=True)
    return accelerator
    ...
    with torch.no_grad():
        for token_id in placeholder_token_ids:
            if opt.initializer_token:
                ...
                token_embeds[token_id] = torch.randn_like(token_embeds[token_id])
...
def save(
    ...
    style_num = opt.style_path.split('/')[-2] 
    torch.save(embeds_dict, ospj(opt.output_dir, 'style_'+style_num, "embedding", f"{prefix}.bin"))
...
def get_options():
    ...
    parser.add_argument(
        "--style_path",
        type=str,
        default='../../dataset/style/wikiart', #52,20,38,83,79 - wikiart // 1003,1004,1005 - inst
...
if __name__ == "__main__":
    ...
    for style_num in opt.style_nums.split(','):
        style_dir = os.path.join(opt.style_path, style_num)
        args.style_path = os.path.join(style_dir, os.listdir(style_dir)[0])
