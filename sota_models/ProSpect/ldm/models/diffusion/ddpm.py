...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        batch_size = x_start.shape[0]
        model_output = self.apply_model(x_noisy, t, cond[int(t*self.prospect_stages/1000)])
        ...
        if self.embedding_reg_weight > 0:
            loss_embedding_reg = self.embedding_manager.embedding_to_coarse_loss()
    ...
    @torch.no_grad()
    def log_images(self, batch, N=1, n_row=4, sample=True, ddim_steps=50, ddim_eta=1., return_keys=None,
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            # c_concat = [x] # InstructP2P
            c_concat = [torch.zeros_like(x)] # InstructP2P
            print('c_concat.shape',x.shape)
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
