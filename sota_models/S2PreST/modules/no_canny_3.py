            ...
            with model.ema_scope():
                ...
                for n in trange(n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        x_noisy = x_noisy_content
                        model_output = model.apply_model(x_noisy, torch.tensor([t_enc]*batch_size).to(device), c_con)
                        ...
                        t_enc = int(strength * ddim_steps)
                        samples = sampler.decode(z_enc, c_con, t_enc, 
                                                unconditional_guidance_scale=scale,
                                                 unconditional_conditioning=uc_con,)
